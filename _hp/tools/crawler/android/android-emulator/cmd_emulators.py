...
def grant_location_permission(device, package_name):
	cmd = [f'{PLATFORM_PATH}/adb', '-s', device, 'shell', 'pm', 'grant', '-g', package_name, 'android.permission.ACCESS_FINE_LOCATION']
...
def force_stop_all_emulators():
	kill_current_devices_cmd = "kill -9 $(ps aux | grep '[q]emu-system-i386-headless @device_' | awk '{print $2}')"
	subprocess.run(kill_current_devices_cmd, shell=True)
	time.sleep(2)	
	kill_current_devices_cmd = "kill -9 $(ps aux | grep '[q]emu-system-x86_64-headless @device_' | awk '{print $2}')"
	subprocess.run(kill_current_devices_cmd, shell=True)
	time.sleep(2)	
	kill_current_devices_cmd = "kill -9 $(ps aux | grep '[q]emu-system-i386 @device_' | awk '{print $2}')"
	subprocess.run(kill_current_devices_cmd, shell=True)
	time.sleep(2)	
	kill_current_devices_cmd = "kill -9 $(ps aux | grep '[q]emu-system-x86_64 @device_' | awk '{print $2}')"
	subprocess.run(kill_current_devices_cmd, shell=True)
	time.sleep(2)	

def start_emulators(num_devices, has_window = False):
	port = 5554
	for device in range(1,num_devices + 1):		
		...
		if has_window:
			print('here')
			cmd_text = f'nohup {EMULATOR_PATH}/emulator @device_{device} -no-snapshot -memory 4096 -verbose -port {port}&'		
		subprocess.run(cmd_text, shell=True)				
		port += 2
		time.sleep(3)
...
def main():
	...
	ap.add_argument('-has_window', '--has_window', dest='has_window', action='store_true')
	ap.set_defaults(has_window=False)
	...
	if args.method == 'start':
		start_emulators(args.num_devices, args.has_window)
