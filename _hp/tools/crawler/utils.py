    ...
    for url, label, num_resp_ids in test_info:
        if "upgrade" in url and scheme == "https":
            ...
            for first_id, last_id in get_resp_ids(label, resp_type, num_resp_ids):
                test_urls.append(f"{scheme}://{base_host}/{base_dir}/{url}?resp_type={resp_type}&browser_id={browser_id}&label={label}&first_id={first_id}&last_id={last_id}")
    return test_urls


def get_resp_ids(label, resp_type, num_resp_ids):
    resp_ids = httpx.get(f"https://{base_host}/_hp/server/get_resp_ids.py?label={label}&resp_type={resp_type}").json()
